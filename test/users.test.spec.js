    ...
    it('Should not update profile if invalid token is provided', (done) => {
      chai
        ...
        .end((err, res) => {
          expect(res).to.have.status(401);
          expect(res.body).to.be.an('object');
          expect(res.body.message).to.equal('Failed to authenticate token! Valid token required');
          done();
        });
    });
    it("should return successfully when a user's profile is successfully updated", (done) => {
      chai
        .request(app)
        .put('/api/v1/users/update')
